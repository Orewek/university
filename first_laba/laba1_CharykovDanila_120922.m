...
%{
imagesc
Используя всю цветовую палитру, расскашивает
DataFrame, либо иной график.
Помогает отчетливее увидеть отклонения
%}
...
Подтвержедние основного тригонометрического тождества
%}
...
Приведенные формулы в качестве доказательства
%}
...
Полученный результат совпадает с корнем числа 555
%}
...
%{
8
Ближайшим корнем уравнения является: x = 3.1238
Проходимся по циклу от 2 до 4 с шагом 10^-6
сначала берем 5000, считаем значениие выражения при х = 2
...
res - наиближайшее расстояние до 0
temp_res -  пока проходимся по циклу мы можем найти расстоние
меньше чем мы нашли во все прошлые разы. Если находим, то 
записываем в параметр res
final_x - после того, как мы в последний раз нашли 
минимальное расстояние до x (параметр res хранит это значение),
мы должны записать число x, при котором это было достигнуто
